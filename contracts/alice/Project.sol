...
contract Project is Ownable {
    ...
    constructor(string _name, uint8 _upfrontPaymentPercentage) public {
        require(
            _upfrontPaymentPercentage >= 0 && _upfrontPaymentPercentage < 100
        );
        name = _name;
    ...
    function registerDonation(address _from, uint256 _amount) internal {
        ...
        ImpactRegistry(IMPACT_REGISTRY_ADDRESS).registerDonation(
            _from,
            remainder
        );
    ...
    function validateOutcome(bytes32 _claimId, uint256 _value) public {
        ...
        ImpactRegistry(IMPACT_REGISTRY_ADDRESS).registerOutcome(
            _claimId,
            _value
        );
    }
