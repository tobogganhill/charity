...
contract ImpactRegistry is Ownable {
    ...
    /* Structure to help iterate over donations */
    ...
    /* Structure stores a match between validated outcome and donations */
    ...
    function registerDonation(address _from, uint256 _value) public onlyMaster {
    ...
    function registerOutcome(bytes32 _claimId, uint256 _value)
        external
        onlyMaster
    {
    ...
    function updateBalance(uint256 _index, uint256 _linkedValue) internal {
        uint256 oldBalance = accountBalances[accountIndex[_index]];
        uint256 newBalance = oldBalance.sub(_linkedValue);
    ...
    function getBalance(address _donorAddress) public view returns (uint256) {
        return accountBalances[_donorAddress];
    }

    function getImpactCount(bytes32 _claimId) public view returns (uint256) {
        return impacts[_claimId].count;
    }

    function getImpactLinked(bytes32 _claimId) public view returns (uint256) {
        return impacts[_claimId].linked;
    }

    function getImpactTotalValue(bytes32 _claimId)
        public
        view
        returns (uint256)
    {
    ...
    function getImpactUnmatchedValue(bytes32 _claimId)
        public
        view
        returns (uint256)
    {
    ...
    function getImpactDonor(bytes32 _claimId, uint256 index)
        public
        view
        returns (address)
    {
    ...
    function getImpactValue(bytes32 _claimId, address addr)
        public
        view
        returns (uint256)
    {
